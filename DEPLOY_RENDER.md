...
5. Deploy. After the service is live Render will provide a public URL like `https://survly-backend.onrender.com`.

6. Update the frontend `VITE_API_BASE` to the Render URL (no trailing slash) and rebuild the `survly` app:

```bash
# Windows PowerShell
$env:VITE_API_BASE='https://survly-backend.onrender.com'
